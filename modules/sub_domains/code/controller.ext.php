...
class module_controller extends ctrl_module
{
    ...
    static function doCreateSubDomain()
    {
        ...
        if (self::ExecuteAddSubDomain($currentuser['userid'], $formvars['inSub'] . "." . $formvars['inDomain'], $formvars['inDestination'], $formvars['inAutoHome'])) {
						...
						$sql = $zdbh->prepare("SELECT vh_id_pk FROM `x_vhosts` WHERE vh_type_in = 1 and vh_name_vc = :domain LIMIT 1");
						$sql->bindParam(":domain", $formvars['inDomain']);
						$sql->execute();
						$result = $sql->fetch(PDO::FETCH_BOTH);
						$record = [
							"uid" => $currentuser['userid'],
							"domainName" => $formvars['inDomain'],
							"domainID" => $result[0],
							"type" => "CNAME",
							"hostName" => $formvars['inSub'],
							"ttl" => 3600,
							"target" => "@"
						];
						self::createDNSRecord($record);
						self::$ok = TRUE;
            return true;
        } else {
            return false;
